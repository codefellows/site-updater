...
const tmpDir = process.env.TMP_DIR || sh.tmpDir()
...
module.exports = function updater() {
  ...
  if (cloneResult === null) {
    ...
    console.log(cloneResult.output)
    ...
    if (commitResult.code === 0) {
      ...
      console.log(`commit failed: ${commitResult.output}`)
      websiteChannel({ text:
        'Site updater problem, commit failed.',
      })
    }
  } else {
    console.log(`clone failed ${cloneResult.output}`)
    websiteChannel({
      text: 'clone failed',
    })
  }
